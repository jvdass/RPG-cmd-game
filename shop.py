...
def open_shop(player):
    ...
    while True:
        print(f"\n{Style.YELLOW}--- BOUTIQUE (Nomnbre d'or actuel: {player.gold}) ---{Style.RESET}")
        for k, v in items.items(): print(f"{k}. {v['name']} ({v['price']} po)")
        c = int(input("Acheter (1-3) ou (0)uitter : ").upper())
        if c == 0 : break
        if c in items:
            itm = items[c]
            ...
            else: print(f"{Style.RED}Pas assez d'or !{Style.RESET}")